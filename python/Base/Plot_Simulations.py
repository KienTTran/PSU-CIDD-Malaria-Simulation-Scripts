...
#Data vars
def plot_simulations(rep_file, delay, start_year, end_year, year_interval):
    rep_tag = ""  
    rep_tags = []
    print(rep_file)
    ...
    for data in rep_data_observe:
        ...
        data["1:Allele_K"] = (data["0:KYY--C1x"] + data["0:KYY--Y1x"])/sum
        data["1:Allele_C"] = (data["0:KYY--C1x"] + data["0:TYY--C1x"])/sum
        data["1:Allele_T"] = (data["0:TYY--C1x"] + data["0:TYY--Y1x"])/sum
        data["1:Allele_Y"] = (data["0:KYY--Y1x"] + data["0:TYY--Y1x"])/sum
        ...
        data.iloc[:, 12:] = data.iloc[:, 4:].div(sum, axis='index')
    ...
    if rep_x_observe == "Year":
        x_tick_observe = range(0,rep_data_range[1] - rep_data_range[0],year_interval)  
    else:
    ...
    rep_header_observe = rep_headers[8:16]
