...
exp_number = 'Calibrate_Z_Kappa_Re_Run_Merged'

local_path = "D:\\plot\\Validation\\Calibrate_Z_Kappa\\merged\\"
local_path_output = local_path + "\\output"

config_df = pd.read_csv(local_path + 'configs.csv',index_col='Index')
...
for index,config in config_df.iterrows():
    for run in range(n_run):
        ...
        beta = config.beta
        # print(beta,z,kappa)        
        file_path_summary = os.path.join(local_path_output, filename_summary)
        file_path_monthly = os.path.join(local_path_output, filename_monthly)
        try:
